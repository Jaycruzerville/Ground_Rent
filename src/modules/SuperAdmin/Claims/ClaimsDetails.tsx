...
const ClaimsDetails = () => {
  ...
  const { data: claimsDetails, isLoading } = useQuery({
    queryKey: ["claims-details", { id }],
    queryFn: SuperAdminService.getClaimsDetails,
  ...
  const claimsColumns: ColumnDef<claimsData>[] = [
    ...
    },
    {
      ...
      header:
        !["Approved", "Rejected"]?.includes(claimsDetails?.data.status) &&
        "Actions",
      cell: () => {
        if (["Approved", "Rejected"]?.includes(claimsDetails?.data.status)) {
          return <Box></Box>
        }
        return (
          <Flex gap="10px">
        ...
        >
          <Text
            ...
            color={
              info.getValue().toLowerCase() === "approved"
                ? "#027A48"
                : info.getValue().toLowerCase() === "arocessing"
                ? "#DC6803"
                : info.getValue().toLowerCase() === "disapproved"
                ? "#D92D20"
                : info.getValue().toLowerCase() === "rejected"
